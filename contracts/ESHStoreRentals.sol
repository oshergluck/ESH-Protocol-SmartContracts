...
contract ESHStoreRentals is ReentrancyGuard {
    ...
    //takes 5% each payment to worker
    ...
    function isClient(address _address) public view returns (bool) {
        return clients[_address].walletAddress != address(0);
    }
    ...
    function refundClient(address _client, uint256 _receiptId) nonReentrant onlyOwner public {
        ...
        // Burn all NFTs associated with this receipt
        uint256[] memory nftIds = receiptNFTs[_receiptId];
        for (uint256 i = 0; i < nftIds.length; i++) {
            invoices.burnNFT(nftIds[i]);
        }
        
        // Clear the NFT array for this receipt
    ...
    function finalizeDistribution(bytes32 distributionId) nonReentrant onlyOwner public returns(bool) {
        ...
        // Mark distribution as finalized
        dist.finalized = true;
    ...
    function registerClient(
        ...
        require(bytes(_phoneNum).length > 0, "");
        require(bytes(_physicalAddress).length > 0, "");
        require(clients[msg.sender].walletAddress == address(0), "Client already registered");
    ...
    function handlePaymentAndReceipt(
        ...
        require(
            tokenContract.transferFrom(client.walletAddress, address(this), amountToPay),
    ...
    function handleRewardDistribution(
        uint256 _amount,
        uint256 totalReward,
        address sender
    ) internal {
        ...
        for (uint256 i = 0; i < _amount; i++) {
            if (rewardsPoolCalc >= 500) {
                uint256 rewardToBuyer = (rewardsPoolCalc * 2) / 100000;
                totalReward += rewardToBuyer;
                rewardsPoolCalc -= rewardToBuyer;
            }
        }

        if (rewardsPool >= totalReward) {
            require(rewardToken.transfer(sender, totalReward), "Reward transfer failed");
    ...
    function handleNFTMinting(
        address sender,
        string memory theMetaData,
        uint256 time,
        string memory bq,
        uint256 newReceiptId
    ) internal {
        uint256[] memory nftIds = new uint256[](1);
        nftIds[0] = invoices.mintNFTWithTimer(sender, theMetaData, time, bq);

        // Store NFT IDs for this receipt
    ...
    function purchaseProduct(
        ...
        uint256 time = _amount * 24 * 60 * 60;
        string memory theMetaData = metadata;

        // Handle Payment and Receipt Creation
        (uint256 newReceiptId, uint256 amountToPay) = handlePaymentAndReceipt(_productBarcode, _amount, sender);
        ...
        if(rewardsPool>100000) {
            handleRewardDistribution(_amount, 0, sender);
        }

        // Handle NFT Minting
        handleNFTMinting(sender, theMetaData, time, _productBarcode, newReceiptId);
    }
    ...
    function deleteCustomer(address _customerAddress) nonReentrant public {
