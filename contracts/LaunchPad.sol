...
contract UltraShop is ReentrancyGuard, Ownable {
    ...
    function depositCoin(
        ...
        // Check if tokenA is a valid ERC20 token
        require(IERC20(tokenA).totalSupply() > 0, "Invalid ERC20 token");

        // Test if it's an ESH token by trying to call getHolders()
        try IESH(tokenA).getHolders() returns (address[] memory holders) {
            ...
            bool creatorIsHolder = IESH(tokenA).isReady();
            require(creatorIsHolder, "Creator must be a token holder");
            
        } catch {
            revert("Token must be an ESH-compatible token with getHolders() function");
        }

        // Transfer the required amount of tokenA from creator to contract
        require(
            IERC20(tokenA).transferFrom(msg.sender, address(this), REQUIRED_TOKEN_A_AMOUNT),
            "TokenA transfer failed"
        );

        // Calculate how many tokens the creator gets for their initial USDC
        // Using proper bonding curve formula: tokens = (totalTokens * usdcAmount) / (usdcAmount + initialReserve)
        uint256 tokenAmount = (REQUIRED_TOKEN_A_AMOUNT * initialUSDC) / (initialUSDC + INITIAL_PRICE * REQUIRED_TOKEN_A_AMOUNT / 10**18);
        ...
        // Initialize coin with proper bonding curve reserves
        uint256 initialTokenReserve = REQUIRED_TOKEN_A_AMOUNT - tokenAmount;
        
        coins[tokenA] = Coin({
            tokenA: tokenA,
            tokenAReserve: initialTokenReserve,
            ...
            creatorFeesUSDC: 0,
            creatorFeesTokenA: 0,
            virtualTokenReserve: initialTokenReserve,
            virtualUSDCReserve: initialUSDC + (6000*1e6)
        });
        ...
        // Transfer the purchased tokens to the creator
        require(
            IERC20(tokenA).transfer(msg.sender, tokenAmount),
            "Token transfer to creator failed"
        );
        ...
        emit CoinDeposited(msg.sender, tokenA, REQUIRED_TOKEN_A_AMOUNT, initialUSDC, price);
    }
