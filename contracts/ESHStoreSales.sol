...
contract ESHStoreSales is ReentrancyGuard {
    ...
    uint256 constant receiptIdFromUltimateDeal = 1;
    ...
    function purchaseProduct(string memory _productBarcode,uint256 _amount, string memory _info, string memory metadata) nonReentrant public {
        ...
        string memory bq = _productBarcode;
        ...
        // Perform external interactions after state changes
        require(tokenContract.transferFrom(client.walletAddress, address(this), amountToPay), "Token transfer failed");
        ...
        for (uint256 i = 0; i < _amount; i++) {
            uint256 nftId = invoices.mintNFT(msg.sender, metadata, bq);
            nftIds[i] = nftId;
        }
        if(rewardsPool>=totalReward&&totalReward!=0) {
